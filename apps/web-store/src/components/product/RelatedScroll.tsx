        ...
        >
          {items.map((product, index) => {
            const image =
              ...
              ''
            return (
              <div key={`${product._id}-${index}`} style={{ width: CARD_WIDTH - 16, flex: '0 0 auto' }}>
                <Link href={`/shop/${product.slug}`} style={{ textDecoration: 'none', display: 'block' }}>
                  <div style={{ aspectRatio: '4/5', width: '100%', overflow: 'hidden', marginBottom: 8 }}>
                    {image
                      ? <img src={image} alt={product.name} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                      : <div style={{ background: 'var(--rych-lift)', width: '100%', height: '100%' }} />
                    }
                  </div>
                  <div
                    style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontFamily: 'var(--rych-font-sans)', fontSize: 11, color: 'var(--rych-ash)' }}>
                      {formatNPR(product.sellingPrice)}
                    </span>
                    {product.marketPrice && product.marketPrice > product.sellingPrice && (
                      <span style={{ fontFamily: 'var(--rych-font-sans)', fontSize: 10, color: 'var(--rych-smoke)', textDecoration: 'line-through' }}>
